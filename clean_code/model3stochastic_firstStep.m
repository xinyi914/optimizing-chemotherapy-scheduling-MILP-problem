% This file is for implementing the first step in the model of two step model in stochastic case 
...
p_list = [p1];
% p_list_str = getVarNames(p13); %change when the p_list change

% loop for each day and save results to txt file for each day
for p_idx = 1:size(p_list,2)
% 
%save log file to the folder
% str_p = sprintf('%s', p_list_str(p_idx)); % for running all in plist
% str_p = 'p1'; % for running only one p
% dfile=strcat(pwd,'/variable_mat_files_step_stochastic_sampling/diary/',str_p,'.txt');
% if exist(dfile,'file'); delete(dfile); end
% diary(dfile)
% diary on
tic;
...
% a = [30;30;60;30;30;60;180;180;30;60;60;120;30;120;60;30:30];

B=14; % number of beds used
...
% variables in stochastic case
% % read mu and variance from the excel
% data = load("data.mat");
...
% % 
% % 
% % means = [str2double(data_30{1});str2double(data_60{1});str2double(data_120{1});str2double(data_180{1});str2double(data_240{1});str2double(data_300{1});str2double(data_360{1})];
% % mu = means;
% % vars = [str2double(data_30{2}),str2double(data_60{2}),str2double(data_120{2}),str2double(data_180{2}),str2double(data_240{2}),str2double(data_300{2}),str2double(data_360{2})];
% % cov = diag(vars);
% % % cov = eye(K);
% 
% mu = data.means;
% mu = transpose(mu);
% cov = diag(variances);
alpha = 0.05; % probability that the apointment is not fulfilled as expected
...
cvx_begin
% cvx_solver_settings('MSK_DPAR_MIO_TOL_REL_GAP', 20e-3)
...
for k=1:K
    ...
    for b=1:B
        ...
        mu'*f(:,b) + sqrt((1-alpha)/alpha)*norm(sqrt(cov)*f(:,b)) <= omega(b)*theta(b);
...
% filename = strcat(pwd,'/variable_mat_files_step_stochastic_sampling/',str_p,'.mat'); %file + # of bed + # of slots + # of appointments.mat
% save(filename)
% 
% % get optimizer time
% filename = dfile;
...
% writecell(c,'first_step_stochastic result/model3_firstStep_sampling_performance.xlsx','WriteMode','append');
end
